  ...
  pages: {
    ...
    channels: {
      ...
      placeholders: {
        ...
        accountAgent: '留空则不为该账号显式绑定 Agent',
        accountPrimaryModel: '留空则使用默认模型链路',
        accountFallbackModels: '为该账号选择回退模型',
        accountId: '输入新的 accountId，例如 default / bot-2',
        accountField: '输入 {field}',
        secret: '输入新值（留空不变更）',
      },
      accountAgentBound: '当前账号绑定到：{agent}',
      accountAgentEmpty: '当前账号还没有显式绑定 Agent。',
      accountAgentUnbound: '未绑定 Agent',
      accountPrimaryModelBound: '主模型：{model}',
      accountPrimaryModelEmpty: '主模型：使用默认配置',
      accountFallbackModelsBound: '回退模型：{models}',
      accountFallbackModelsEmpty: '回退模型：无',
      missingAgent: '已不存在的 Agent',
      missingModel: '已不存在的模型',
      agentLoadFailed: '读取 Agent 列表失败：{error}',
